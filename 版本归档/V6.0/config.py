"""
V6.0 策略配置文件 —— Evidence Engine + EmotionBuilder + Replay Learning
==========================================================================

V6.0 核心升级：
- Evidence Engine（多源置信度融合：Rule + Replay + ML + Emotion）
- EmotionBuilder（多源数据融合情绪引擎，替代纯价格指标情绪判断）
- Replay Learning（行为成功率统计，纯统计无ML）
- Time Decay（观察期置信度衰减）
- ML Confidence（机器学习概率输出）
- Probability Calibration（概率校准）
- LLM Sentiment Provider 接口（预留）
...
EVIDENCE_ENABLE_ML = False        # ML默认关闭（需要训练数据积累）
EVIDENCE_ENABLE_EMOTION_BONUS = True  # 情绪双确认

# ============================================================
# V6.0 新增：Replay Learning (Behavior Memory) 参数
# ============================================================
REPLAY_WINDOW_DAYS = 90           # 滑动窗口大小（交易日）
REPLAY_MIN_SAMPLES = 5            # 最小样本量（低于此值使用中性系数）
REPLAY_MAX_AGE_DAYS = 30          # 超过此天数无新样本 → 回归中性

# 成功率 → 置信度乘数映射
REPLAY_HIGH_SUCCESS = 0.70        # 成功率高于此值 → 加分
REPLAY_LOW_SUCCESS = 0.40         # 成功率低于此值 → 减分
REPLAY_CONSECUTIVE_FAIL = 3       # 连续N次失败 → 紧急降权

# ============================================================
# V6.0 新增：Time Decay 参数
# ============================================================
TIME_DECAY_GRACE_PERIOD = 5       # 前N天不衰减（宽限期）
TIME_DECAY_RATE = 0.05            # 每天衰减比例（5%）
TIME_DECAY_MIN_CONFIDENCE = 25    # 衰减到此值以下 → 过期标记
