"""
V6.0 事件引擎（Event Engine）+ 行为生命周期（Behavior Lifecycle）
================================================================

V6.0 升级：
- 集成 Time Decay（观察期置信度时间衰减）
...
class BehaviorEvent:
    ...
    def update(self, date, new_evidence, regime, psychology_state, df_index_data):
        ...
        if self.current_state == 'Candidate':
            ...
            # 判断是否超过观察窗口
            if self.age > OBSERVATION_WINDOW_MAX:
            ...
            # 即使在窗口内，如果置信度足够高也可以提前确认
            elif self.age >= OBSERVATION_WINDOW_MIN and self.confidence >= CONFIRMATION_THRESHOLD + 10:
                self.current_state = 'Confirmed'
