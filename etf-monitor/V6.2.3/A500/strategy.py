"""
V6.2.1 策略流水线（Strategy Pipeline）—— 稳定化版本
===================================================

完整的 V6.1 架构：

  数据预处理（indicators.py）
        ↓
  Market Regime（regime_detector.py）
        ↓
  Behavior Detection（behavior_detector.py）
        ↓
  EmotionBuilder（emotion_builder.py）
        ↓
  Event Engine + Lifecycle（event_engine.py）
        ↓
  Evidence Engine（evidence_engine.py）—— V6.1 在线学习闭环
        ↓
  ML Confidence（ml_confidence.py）—— V6.1 真正启用
        ↓
  Probability Calibration（probability_calibration.py）—— V6.1 评估
        ↓
  Reward / Risk Evaluation（reward_risk.py）
        ↓
  Scoring Engine（scoring_engine.py）
        ↓
  Position Manager（position_manager.py）

V6.1 关键变化：
- Time Decay: exp(-days/τ) 替代 (1-rate)^n
- Replay: (Regime, Behavior, Psychology) 三维学习 + Laplace平滑
- ML: 默认启用，每N笔交易后在线重训练
- Calibration: Brier/LogLoss评估，校准变差自动禁用
"""

import pandas as pd
import numpy as np
from datetime import datetime, time

from indicators import calculate_indicators
from regime_detector import detect_market_regime
from behavior_detector import detect_all_behaviors
from emotion_builder import EmotionBuilder
from event_engine import EventEngine
from evidence_engine import EvidenceEngine
from reward_risk import evaluate_reward_risk
from scoring_engine import calculate_final_score, score_to_target_position, get_score_to_action
from position_manager import PositionManager
from ml_confidence import MLConfidence, default_feature_builder
from probability_calibration import ProbabilityCalibrator
from config import *

# 主要节假日列表（2026年）
MAJOR_HOLIDAYS_2026 = [
    '2026-01-01',
    '2026-02-01', '2026-02-02', '2026-02-03', '2026-02-04', '2026-02-05',
    '2026-04-04', '2026-04-05',
    '2026-05-01', '2026-05-02', '2026-05-03', '2026-05-04', '2026-05-05',
    '2026-06-25', '2026-06-26',
    '2026-09-27',
    '2026-10-01', '2026-10-02', '2026-10-03', '2026-10-04', '2026-10-05', '2026-10-06', '2026-10-07',
]


def is_trading_day(date):
    """判断是否为交易日"""
    if isinstance(date, str):
        date = datetime.strptime(date, '%Y-%m-%d').date()
    elif isinstance(date, datetime):
        date = date.date()
    if date.weekday() >= 5:
        return False
    if date.strftime('%Y-%m-%d') in MAJOR_HOLIDAYS_2026:
        return False
    return True


class V6Strategy:
    """
    V6.1 策略流水线（稳定化版本）

    升级：
    - ML默认启用，每N笔交易后在线重训练
    - Replay传入Psychology维度
    - Calibration评估自动禁用
    - Evidence Explainability增强
    """

    def __init__(self, use_ml=True, emotion_method='weighted'):
        """
        Args:
            use_ml: 是否启用ML证据源（V6.1默认True）
            emotion_method: 情绪融合方法
        """
        self.event_engine = EventEngine()
        self.emotion_builder = EmotionBuilder(method=emotion_method)
        self.evidence_engine = EvidenceEngine(
            weights=EVIDENCE_WEIGHTS.copy(),
            use_ml=EVIDENCE_ENABLE_ML if use_ml else False,
            use_replay=EVIDENCE_ENABLE_REPLAY,
        )
        self.signals = []
        self.replay_records = []

        # V6.1: ML 模型和校准器
        self.ml_confidence = None
        self.calibrator = None
        self.ml_trade_count = 0  # 用于跟踪训练次数
        self._ml_training_data = []  # 累积训练数据
        self.use_ml = use_ml

        # V6: 价格+情绪双确认记录
        self.emotion_confirmation_log = []

        # V6.2.1: 牛市止盈后重新入场追踪
        self._reentry_until_idx = -1  # 直入窗口结束的索引位置
        self._last_sell_signal_idx = -1
        self._reentry_log = []

    def fit_emotion_builder(self, df):
        """使用历史数据拟合EmotionBuilder（PCA/ICA方法）"""
        if self.emotion_builder.method in ('pca', 'ica'):
            self.emotion_builder.fit(df)

    def run(self, df, start_date=None):
        """
        运行 V6.0 完整策略流水线

        Args:
            df: 包含 OHLCV 的 DataFrame（已计算指标）
            start_date: 回测起始日期

        Returns:
            signals: list of dict
        """
        # Step 0: 确保指标已计算（已有则跳过）
        if 'ma5' not in df.columns:
            df = calculate_indicators(df)

        # V6: 拟合EmotionBuilder（如果需要）
        self.fit_emotion_builder(df)

        self.signals = []
        self.replay_records = []

        for i in range(len(df)):
            current_date = df.index[i]
            date_str = current_date.strftime('%Y-%m-%d') if hasattr(current_date, 'strftime') else str(current_date)

            # 非交易日跳过
            if not is_trading_day(current_date):
                self.signals.append({
                    'date': current_date,
                    'regime': 'NonTrading',
                    'psychology': self.emotion_builder.get_state(),
                    'buy_score': 0,
                    'sell_score': 0,
                    'target_position': None,
                    'action': 'HOLD',
                    'description': '非交易日',
                })
                continue

            # 数据不足
            if i < 60:
                self.signals.append({
                    'date': current_date,
                    'regime': 'Insufficient',
                    'psychology': 'Unknown',
                    'buy_score': 0,
                    'sell_score': 0,
                    'target_position': None,
                    'action': 'HOLD',
                    'description': '数据不足',
                })
                continue

            # ---- Layer 1: Market Regime ----
            regime = detect_market_regime(df, i)

            # ---- Layer 2: Behavior Detection ----
            behavior_result = detect_all_behaviors(df, i, regime)

            # ---- Layer 3: EmotionBuilder Update (替代 CrowdPsychology) ----
            psych_state, psych_changed, psych_desc = self.emotion_builder.update(df, i, current_date)
            emotion_score = self.emotion_builder.get_emotion_score()
            emotion_improving, emotion_magnitude = self.emotion_builder.get_emotion_improvement(
                EMOTION_IMPROVEMENT_WINDOW
            )

            # V6: 价格+情绪双确认记录
            if emotion_improving and behavior_result['buy_behaviors']:
                self.emotion_confirmation_log.append({
                    'date': current_date,
                    'behavior': behavior_result['buy_behaviors'][0][0],
                    'emotion_from': self.emotion_builder.state_history[-EMOTION_IMPROVEMENT_WINDOW][1] if len(self.emotion_builder.state_history) > EMOTION_IMPROVEMENT_WINDOW else 'N/A',
                    'emotion_to': psych_state,
                    'magnitude': round(emotion_magnitude, 2),
                })

            # ---- Layer 4: Event Engine (Lifecycle) with Time Decay ----
            daily_summary = self.event_engine.process_daily(
                current_date, behavior_result, regime, psych_state, df, i
            )

            # ---- Layer 5: Reward / Risk Evaluation ----
            reward_score, risk_score, reward_detail, risk_detail = evaluate_reward_risk(
                df, i, regime, psych_state
            )

            # ---- Layer 6: V6 Evidence Engine ----
            # V6.2.1: 牛市止盈后重新入场检查
            in_reentry_window = BULL_REENTRY_ENABLED and (i <= self._reentry_until_idx)
            if in_reentry_window:
                reentry_days_left = self._reentry_until_idx - i
            else:
                reentry_days_left = -1

            buy_score, sell_score, score_breakdown, evidence_debug = self._compute_v6_score(
                reward_score, risk_score, psych_state, regime,
                emotion_score, emotion_improving, emotion_magnitude,
                current_date, df_slice=df, idx=i,
                bull_reentry_boost=BULL_REENTRY_BUY_BOOST if in_reentry_window else 1.0
            )

            # V6.2.1: 在牛市中买入评分突然大幅下降 → 视为止盈信号，开启直入窗口
            # 使用 buy_score 而非 sell_score，因为 Bull 中 sell 被压得很低
            if BULL_REENTRY_ENABLED and regime == 'Bull':
                prev_buy = self.signals[-1]['buy_score'] if self.signals else 0
                buy_drop = prev_buy - buy_score
                # 买入评分突然下降超过阈值 → 策略正在卖出/减仓
                if buy_drop > BULL_REENTRY_SCORE_DROP and buy_score < BULL_REENTRY_SCORE_LOW:
                    self._reentry_until_idx = i + BULL_REENTRY_WINDOW
                    self._last_sell_signal_idx = i
                    self._reentry_log.append({
                        'date': date_str,
                        'reentry_until_idx': self._reentry_until_idx,
                        'prev_buy': prev_buy,
                        'buy_score': buy_score,
                        'sell_score': sell_score,
                        'drop': buy_drop,
                    })

            # ---- Layer 7: Position (handled by backtest engine) ----

            # ---- 构建 Replay 记录 ----
            replay = self._build_replay_record(
                current_date, regime, psych_state, psych_changed, psych_desc,
                daily_summary, buy_score, sell_score, score_breakdown,
                reward_score, risk_score, evidence_debug
            )
            self.replay_records.append(replay)

            # ---- 构建信号 ----
            buy_behaviors = [b[0] for b in behavior_result.get('buy_behaviors', [])]
            sell_behaviors = [s[0] for s in behavior_result.get('sell_behaviors', [])]

            signal = {
                'date': current_date,
                'regime': regime,
                'psychology': psych_state,
                'emotion_score': emotion_score,
                'emotion_improving': emotion_improving,
                'buy_score': buy_score,
                'sell_score': sell_score,
                'reward_score': reward_score,
                'risk_score': risk_score,
                'buy_behaviors': buy_behaviors,
                'sell_behaviors': sell_behaviors,
                'confirmed_buy_events': len(score_breakdown['buy']['events']),
                'confirmed_sell_events': len(score_breakdown['sell']['events']),
                'active_events': daily_summary['active_count'],
                'score_breakdown': score_breakdown,
                'evidence_debug': evidence_debug,
                'replay': replay,
                # V6.2.1: 牛市直入状态
                'bull_reentry': in_reentry_window,
                'reentry_days_left': reentry_days_left,
            }
            self.signals.append(signal)

        return self.signals

    def _compute_v6_score(self, reward_score, risk_score, psych_state, regime,
                           emotion_score, emotion_improving, emotion_magnitude,
                           current_date, df_slice=None, idx=None,
                           bull_reentry_boost=1.0):
        """
        V6.0 评分计算 + V6.1 历史高位保护 + V6.2.1 牛市直入

        Returns:
            buy_score, sell_score, score_breakdown, evidence_debug
        """
        from indicators import get_value

        confirmed_buy_events = self.event_engine.get_confirmed_buy_events()
        confirmed_sell_events = self.event_engine.get_confirmed_sell_events()

        # ============================================================
        # V6.1: 历史高位保护 —— 距60日高点太近时抑制买入
        # 核心逻辑：在 Euphoria/Exhaustion 时严格禁止高位买入；
        #           在 Bull+Hope 时则放松（牛市持续创新高是正常的）
        # ============================================================
        near_ath_suppression = 1.0
        if df_slice is not None and idx is not None and idx >= 60:
            try:
                close_val = get_value(df_slice, idx, 'close')
                if close_val:
                    high_60d = df_slice['high'].iloc[max(0, idx-60):idx+1].max()
                    dist_from_high = (close_val - high_60d) / high_60d

                    if dist_from_high > -0.03:    # 距60日高点不到3%
                        if psych_state in ('Euphoria', 'Exhaustion'):
                            near_ath_suppression = 0.10  # 情绪极端+价格高位 → 几乎禁止
                        elif regime == 'Bull':
                            near_ath_suppression = 0.50  # 牛市中允许接近新高
                        else:
                            near_ath_suppression = 0.25
                    elif dist_from_high > -0.06:  # 距60日高点不到6%
                        if psych_state in ('Euphoria', 'Exhaustion'):
                            near_ath_suppression = 0.35
                        else:
                            near_ath_suppression = 0.70
            except:
                pass

        # 市场上下文
        market_context = {
            'regime': regime,
            'emotion_state': psych_state,
            'emotion_score': emotion_score,
            'emotion_improving': emotion_improving,
            'emotion_magnitude': emotion_magnitude,
            'reward_score': reward_score,
            'risk_score': risk_score,
        }

        # --- 买入评分 ---
        buy_events_detail = []
        buy_final_conf = 0
        buy_behavior_score = 0
        evidence_debug_buy = []

        for event in confirmed_buy_events:
            buy_behavior_score += event.strength

            # V6: 通过 Evidence Engine 计算最终置信度
            final_conf, ev_breakdown = self.evidence_engine.compute_final_confidence(
                event, market_context, current_date
            )
            buy_final_conf = max(buy_final_conf, final_conf)

            buy_events_detail.append({
                'name': event.behavior_name,
                'strength': event.strength,
                'confidence': round(final_conf, 1),
                'evidence_breakdown': ev_breakdown,
                'psych_change': event.psych_change,
            })
            evidence_debug_buy.append(ev_breakdown)

        buy_behavior_norm = min(100, buy_behavior_score * 1.2)
        weights = REGIME_WEIGHTS.get(regime, REGIME_WEIGHTS['Unknown'])

        raw_buy = (
            buy_behavior_norm * SCORE_BEHAVIOR_WEIGHT +
            buy_final_conf * SCORE_CONFIDENCE_WEIGHT +
            reward_score * (SCORE_REWARD_WEIGHT * 3)   # reward权重
        )
        buy_score = raw_buy * weights['buy_mult']

        # --- 卖出评分 ---
        sell_events_detail = []
        sell_final_conf = 0
        sell_behavior_score = 0
        evidence_debug_sell = []

        for event in confirmed_sell_events:
            sell_behavior_score += event.strength

            final_conf, ev_breakdown = self.evidence_engine.compute_final_confidence(
                event, market_context, current_date
            )
            sell_final_conf = max(sell_final_conf, final_conf)

            sell_events_detail.append({
                'name': event.behavior_name,
                'strength': event.strength,
                'confidence': round(final_conf, 1),
                'evidence_breakdown': ev_breakdown,
                'psych_change': event.psych_change,
            })
            evidence_debug_sell.append(ev_breakdown)

        sell_behavior_norm = min(100, sell_behavior_score * 1.2)
        raw_sell = (
            sell_behavior_norm * SCORE_BEHAVIOR_WEIGHT +
            sell_final_conf * SCORE_CONFIDENCE_WEIGHT +
            risk_score * (SCORE_RISK_WEIGHT * 9)  # risk权重
        )
        sell_score = raw_sell * weights['sell_div']

        # ============================================================
        # V6.1: 历史高位保护 —— 在情绪修正之前先应用
        # ============================================================
        if near_ath_suppression < 1.0:
            buy_score *= near_ath_suppression
            sell_score *= 1.10  # 高位时卖出信号稍加强

        # --- 情绪修正 ---
        if psych_state == 'Panic':
            buy_score *= PSYCH_PANIC_BUY_BOOST    # 恐慌中勇敢买入小幅加分
        elif psych_state == 'Euphoria':
            sell_score *= PSYCH_EUPHORIA_SELL_BOOST  # 狂热中卖出更强
            buy_score *= PSYCH_EUPHORIA_BUY_CUT      # 狂热中买入大幅折扣
        elif psych_state == 'Exhaustion':
            sell_score *= PSYCH_EXHAUSTION_SELL_BOOST  # 衰竭中卖出强烈加分

        # V6.2.1: 牛市止盈后重新入场 Boost
        if bull_reentry_boost > 1.0:
            buy_score *= bull_reentry_boost
            sell_score *= (1.0 / bull_reentry_boost)  # 卖出信号相应减弱

        # --- 构建分解 ---
        score_breakdown = {
            'buy': {
                'behavior_score': round(buy_behavior_norm, 1),
                'confidence': round(buy_final_conf, 1),
                'reward_score': round(reward_score, 1),
                'raw_score': round(raw_buy, 1),
                'final_score': round(buy_score, 1),
                'events': buy_events_detail,
                'regime_mult': weights['buy_mult'],
            },
            'sell': {
                'behavior_score': round(sell_behavior_norm, 1),
                'confidence': round(sell_final_conf, 1),
                'risk_score': round(risk_score, 1),
                'raw_score': round(raw_sell, 1),
                'final_score': round(sell_score, 1),
                'events': sell_events_detail,
                'regime_mult': weights['sell_div'],
            },
            'psychology': psych_state,
            'regime': regime,
        }

        evidence_debug = {
            'buy': evidence_debug_buy,
            'sell': evidence_debug_sell,
        }

        return round(buy_score, 1), round(sell_score, 1), score_breakdown, evidence_debug

    def _build_replay_record(self, date, regime, psych_state, psych_changed, psych_desc,
                              daily_summary, buy_score, sell_score, score_breakdown,
                              reward_score, risk_score, evidence_debug):
        """构建完整的交易解释记录（V6增强版）"""
        record = {
            'Date': date.strftime('%Y-%m-%d') if hasattr(date, 'strftime') else str(date),
            'Regime': regime,
            'Psychology': psych_state,
            'EmotionScore': round(self.emotion_builder.get_emotion_score(), 1),
            'Psychology_Change': psych_desc if psych_changed else '维持不变',
            'BuyScore': round(buy_score, 1),
            'SellScore': round(sell_score, 1),
            'RewardScore': round(reward_score, 1),
            'RiskScore': round(risk_score, 1),
        }

        # 买入事件详情（含Evidence Engine分解）
        buy_events = score_breakdown['buy']['events']
        if buy_events:
            record['Buy_Behavior'] = buy_events[0]['name']
            record['Buy_Confidence'] = buy_events[0]['confidence']
            record['Buy_PsychChange'] = buy_events[0].get('psych_change', {}).get('description', '')

            # V6: Evidence Engine 分解
            ev = buy_events[0].get('evidence_breakdown', {})
            if ev:
                record['Buy_Evidence_PreDecay'] = ev.get('pre_decay', 0)
                record['Buy_Evidence_TimeDecay'] = ev.get('decay_multiplier', 1.0)
                record['Buy_Evidence_Final'] = ev.get('final', 0)
        else:
            record['Buy_Behavior'] = 'None'
            record['Buy_Confidence'] = 0
            record['Buy_PsychChange'] = ''
            record['Buy_Evidence_PreDecay'] = 0
            record['Buy_Evidence_TimeDecay'] = 1.0
            record['Buy_Evidence_Final'] = 0

        # 卖出事件详情
        sell_events = score_breakdown['sell']['events']
        if sell_events:
            record['Sell_Behavior'] = sell_events[0]['name']
            record['Sell_Confidence'] = sell_events[0]['confidence']
            record['Sell_PsychChange'] = sell_events[0].get('psych_change', {}).get('description', '')
        else:
            record['Sell_Behavior'] = 'None'
            record['Sell_Confidence'] = 0
            record['Sell_PsychChange'] = ''

        # 新建候选事件
        new_candidates = daily_summary.get('new_candidates', [])
        record['New_Candidates'] = ', '.join([e.behavior_name for e in new_candidates]) if new_candidates else 'None'

        # 过期事件
        expired = daily_summary.get('expired_events', [])
        record['Expired_Events'] = ', '.join([e.behavior_name for e in expired]) if expired else 'None'

        # V6: 情绪双确认
        record['EmotionConfirmation'] = 'Yes' if self.emotion_confirmation_log and \
            str(self.emotion_confirmation_log[-1]['date']) == record['Date'] else 'No'

        return record

    def get_replay_records(self):
        """获取所有回放记录"""
        return self.replay_records

    def get_behavior_memory(self):
        """获取行为记忆库"""
        return self.evidence_engine.get_behavior_memory()

    def get_emotion_confirmation_log(self):
        """获取情绪双确认日志"""
        return self.emotion_confirmation_log

    def get_reentry_log(self):
        """V6.2.1: 获取牛市直入日志"""
        return self._reentry_log

    def record_trade_result(self, regime, behavior_name, success, date, psychology=None):
        """
        V6.1: 记录交易结果到行为记忆库（增加 Psychology 维度）

        由回测引擎在每笔交易完成后调用。
        """
        if self.evidence_engine.behavior_memory:
            self.evidence_engine.behavior_memory.record_trade(
                regime, behavior_name, success, date, psychology
            )

        # V6.1: 累积ML训练数据
        self.ml_trade_count += 1
        if self.use_ml:
            self._maybe_train_ml_model()

    # ============================================================
    # V6.1: ML 在线训练
    # ============================================================

    def add_ml_training_sample(self, features, label):
        """V6.1: 添加ML训练样本"""
        self._ml_training_data.append((features, label))

    def _maybe_train_ml_model(self):
        """V6.1: 检查是否需要训练/重训练ML模型"""
        from config import ML_RETRAIN_FREQUENCY, ML_MIN_TRAINING_SAMPLES

        if len(self._ml_training_data) < ML_MIN_TRAINING_SAMPLES:
            return

        # 每N笔交易后重训练
        if self.ml_trade_count % ML_RETRAIN_FREQUENCY != 0:
            return

        self._train_ml_model()

    def _train_ml_model(self):
        """V6.1.1: 训练ML模型和校准器（使用统一 FeatureBuilder）"""
        from config import ML_MODEL_TYPE, ML_N_ESTIMATORS, ML_MAX_DEPTH, \
            ML_CV_FOLDS, CALIBRATION_METHOD, CALIBRATION_AUTO_DISABLE
        from feature_builder import FeatureBuilder

        if len(self._ml_training_data) < ML_MIN_TRAINING_SAMPLES:
            return

        X = np.array([d[0] for d in self._ml_training_data])
        y = np.array([d[1] for d in self._ml_training_data])

        # V6.1.1: 特征维度一致性检验
        expected_dim = FeatureBuilder.get_feature_count()
        if X.shape[1] != expected_dim:
            print(f"[WARNING] ML训练特征维度不匹配: 实际{X.shape[1]} vs 预期{expected_dim}，跳过训练")
            return

        # 训练ML模型
        self.ml_confidence = MLConfidence(
            model_type=ML_MODEL_TYPE,
            n_estimators=ML_N_ESTIMATORS,
            max_depth=ML_MAX_DEPTH,
        )
        feature_names = FeatureBuilder.get_feature_names()
        self.ml_confidence.fit(X, y, feature_names=feature_names, cv_folds=ML_CV_FOLDS)

        # 训练校准器
        if self.ml_confidence._is_fitted:
            y_prob_raw = self.ml_confidence.predict_proba(X)
            if isinstance(y_prob_raw, np.ndarray) and len(y_prob_raw) > CALIBRATION_MIN_SAMPLES:
                self.calibrator = ProbabilityCalibrator(method=CALIBRATION_METHOD)
                self.calibrator.fit(y, y_prob_raw)

                # V6.1: 若校准变差则自动禁用
                if CALIBRATION_AUTO_DISABLE and not self.calibrator.calibration_improved:
                    self.evidence_engine.calibration_enabled = False
                else:
                    self.evidence_engine.calibration_enabled = True

            # 设置到Evidence Engine
            self.evidence_engine.set_ml_model(self.ml_confidence, self.calibrator)

    def get_ml_summary(self):
        """V6.1: 获取ML模型摘要"""
        if self.ml_confidence is None or not self.ml_confidence._is_fitted:
            return {'status': 'not_trained', 'samples': len(self._ml_training_data)}

        ml_summary = self.ml_confidence.get_summary()
        cal_summary = self.calibrator.get_summary() if self.calibrator else {}
        return {
            'status': 'trained',
            'ml': ml_summary,
            'calibration': cal_summary,
            'total_samples': len(self._ml_training_data),
            'retrain_count': self.ml_trade_count // ML_RETRAIN_FREQUENCY,
            'calibration_enabled': self.evidence_engine.calibration_enabled,
        }


def run_strategy_v6(df, start_date=None, use_ml=True, emotion_method='weighted'):
    """V6.1: 运行策略（ML默认启用）"""
    strategy = V6Strategy(use_ml=use_ml, emotion_method=emotion_method)
    return strategy.run(df, start_date)


def _percentile_rank(value, series):
    """
    计算 value 在 series 中的百分位排名（0-100），用于评分解读。

    Returns:
        (below_pct, top_pct):
          below_pct: 历史中 <= value 的比例（%），如 92.0 表示高于 92% 的历史值
          top_pct:   100 - below_pct，即处于历史前 top_pct %（越小代表越极端/越强）
    """
    arr = np.asarray(series, dtype=float)
    arr = arr[np.isfinite(arr)]
    if arr.size == 0:
        return None, None
    below_pct = float(np.mean(arr <= value) * 100)
    return below_pct, 100.0 - below_pct


def _theoretical_score_max(regime):
    """
    按当前市况计算买入/卖出评分的理论上限（依据 V6 评分公式推导，未含情绪修正）。

    raw_buy  上限 = 行为100*0.30 + 置信度100*0.15 + Reward50*(0.40*3)  = 105
    raw_sell 上限 = 行为100*0.30 + 置信度100*0.15 + Risk50*(0.15*9)   = 112.5
    """
    w = REGIME_WEIGHTS.get(regime, REGIME_WEIGHTS['Unknown'])
    max_buy = (100 * SCORE_BEHAVIOR_WEIGHT + 100 * SCORE_CONFIDENCE_WEIGHT
               + REWARD_SCORE_MAX * SCORE_REWARD_WEIGHT * 3) * w['buy_mult']
    max_sell = (100 * SCORE_BEHAVIOR_WEIGHT + 100 * SCORE_CONFIDENCE_WEIGHT
                + RISK_SCORE_MAX * SCORE_RISK_WEIGHT * 9) * w['sell_div']
    return max_buy, max_sell


def _estimate_reference_position(df, strategy, signals):
    """通过轻量回测估计策略当前参考持仓（口径与回测一致），失败时退回中性持仓 50%"""
    try:
        from backtest import V6Backtest
        bt = V6Backtest(df, strategy=strategy)
        bt.run(signals)
        return bt.pm.position_pct
    except Exception:
        return 0.50


def get_today_signal_v6(df, current_position=None, capital=None):
    """
    获取今日信号（V6.2.2 增强版：交易建议 + 仓位指引 + 评分解读）

    Args:
        df: 含 OHLCV 的 DataFrame（自动计算指标）
        current_position: 当前持仓比例（0-1）；None 时自动采用策略回测口径的最近持仓
        capital: 参考总资金（元），用于换算绝对买卖数量；None 时默认 100000

    Returns:
        (signal_text, detail_text)
    """
    if len(df) < 60:
        return "0%", "数据不足，需要至少60天数据（V6.0）"

    df = calculate_indicators(df)
    strategy = V6Strategy()
    signals = strategy.run(df)

    if not signals:
        return "N/A", "无信号"

    # 取最近一个交易日信号（跳过非交易日/数据不足的占位记录）
    last = next((s for s in reversed(signals)
                 if s.get('regime') not in ('NonTrading', 'Insufficient')), signals[-1])
    if last.get('regime') in ('NonTrading', 'Insufficient'):
        return "HOLD", "当前无有效信号（非交易日或数据不足）"

    buy_score = last['buy_score']
    sell_score = last['sell_score']
    net = buy_score - sell_score

    # ---- 参考持仓与资金 ----
    if current_position is None:
        current_position = _estimate_reference_position(df, strategy, signals)
    current_position = float(max(0.0, min(1.0, current_position)))
    if capital is None:
        capital = 100000.0

    # ---- 交易建议：动作 + 目标仓位 + 调仓幅度 ----
    action, target, _ = get_score_to_action(buy_score, sell_score, current_position)
    delta = target - current_position

    # ---- 绝对数量换算（ETF 整手 = 100 份） ----
    price = None
    try:
        price = float(df.loc[last['date'], 'close'])
    except Exception:
        try:
            price = float(df['close'].dropna().iloc[-1])
        except Exception:
            price = None

    trade_value = abs(delta) * capital
    shares = None
    if price and trade_value > 0:
        shares = int(trade_value / price // 100 * 100)

    # ---- 评分解读：理论上限 + 历史分位 ----
    max_buy, max_sell = _theoretical_score_max(last['regime'])
    valid_sigs = [s for s in signals if s.get('regime') not in ('NonTrading', 'Insufficient')]
    buy_below, buy_top = _percentile_rank(buy_score, [s['buy_score'] for s in valid_sigs])
    sell_below, sell_top = _percentile_rank(sell_score, [s['sell_score'] for s in valid_sigs])

    lines = []

    lines.append("=" * 60)
    lines.append("A500 ETF V6.0 Evidence Engine 策略")
    lines.append("=" * 60)
    date_str = last['date'].strftime('%Y-%m-%d') if hasattr(last['date'], 'strftime') else str(last['date'])
    lines.append(f"信号日期: {date_str}（最近交易日）")
    lines.append(f"市场状态: {last['regime']}")
    lines.append(f"市场情绪: {last['psychology']} (Score: {last.get('emotion_score', 'N/A')})")
    lines.append(f"情绪改善: {'是' if last.get('emotion_improving') else '否'}")
    lines.append(f"买入评分: {buy_score:.1f} | 卖出评分: {sell_score:.1f}")
    lines.append(f"净评分: {net:+.1f}")
    lines.append(f"Reward: {last.get('reward_score', 0):.1f} | Risk: {last.get('risk_score', 0):.1f}")
    lines.append("")

    # ---- 交易建议 ----
    action_cn = {'BUY': '买入', 'SELL': '卖出', 'HOLD': '持有'}.get(action, action)
    lines.append("---- 交易建议 ----")
    lines.append(f"操作建议: {action_cn} ({action})")
    lines.append(f"目标仓位: {target * 100:.0f}%（当前参考持仓 {current_position * 100:.0f}%）")
    if abs(delta) >= 0.005:
        if delta > 0:
            lines.append(f"调仓幅度: 加仓 +{delta * 100:.1f}%")
        else:
            lines.append(f"调仓幅度: 减仓 {delta * 100:.1f}%")
        if price:
            if shares and shares >= 100:
                lines.append(f"仓位换算: 参考资金 {capital:,.0f} 元 -> 买卖约 {shares:,} 份"
                             f"（约 {trade_value:,.0f} 元，现价 {price:.3f}）")
            else:
                lines.append(f"仓位换算: 调仓金额约 {trade_value:,.0f} 元"
                             f"（不足 1 手 100 份，请按现价 {price:.3f} 手动核算）")
        else:
            lines.append(f"仓位换算: 按总资金 {capital:,.0f} 元的 {abs(delta) * 100:.1f}% 调仓")
    else:
        lines.append("调仓幅度: 无（当前仓位已在目标附近）")
    lines.append("")

    # ---- 评分解读 ----
    lines.append("---- 评分解读 ----")
    lines.append(f"买入评分 {buy_score:.1f} / 理论上限 {max_buy:.1f}（{last['regime']} 市况，未含情绪修正）")
    if buy_below is not None:
        strong = '，偏强' if buy_top <= 10 else ''
        lines.append(f"  历史分位: 高于 {buy_below:.1f}% 的交易日（处于前 {buy_top:.1f}%{strong}）")
    lines.append(f"卖出评分 {sell_score:.1f} / 理论上限 {max_sell:.1f}（{last['regime']} 市况，未含情绪修正）")
    if sell_below is not None:
        strong = '，偏强' if sell_top <= 10 else ''
        lines.append(f"  历史分位: 高于 {sell_below:.1f}% 的交易日（处于前 {sell_top:.1f}%{strong}）")
    hold_zone = SCORE_HOLD_ZONE
    if net > hold_zone:
        net_desc = f"净评分 {net:+.1f} 超过观望区（±{hold_zone}）-> 买入信号占优"
    elif net < -hold_zone:
        net_desc = f"净评分 {net:+.1f} 低于观望区（±{hold_zone}）-> 卖出信号占优"
    else:
        net_desc = f"净评分 {net:+.1f} 处于观望区（±{hold_zone}）-> 维持现状"
    lines.append(f"净评分解读: {net_desc}")
    lines.append("")

    buy_behaviors = last.get('buy_behaviors', [])
    sell_behaviors = last.get('sell_behaviors', [])
    if buy_behaviors:
        lines.append(f"候选人行为: {', '.join(buy_behaviors)}")
    if sell_behaviors:
        lines.append(f"候选卖出行为: {', '.join(sell_behaviors)}")
    lines.append(f"已确认买入事件: {last.get('confirmed_buy_events', 0)}")
    lines.append(f"已确认卖出事件: {last.get('confirmed_sell_events', 0)}")
    lines.append(f"活跃事件总数: {last.get('active_events', 0)}")

    # V6: Evidence Engine 分解
    ev_debug = last.get('evidence_debug', {}).get('buy', [])
    if ev_debug and ev_debug[0]:
        ev = ev_debug[0]
        lines.append("")
        lines.append("--- Evidence Engine 融合明细 ---")
        for src, info in ev.get('sources', {}).items():
            contrib = info.get('contribution', 0)
            if contrib != 0:
                lines.append(f"  {src.upper():>8}: {contrib:+.1f}")
        lines.append(f"  Pre-Decay: {ev.get('pre_decay', 0):.1f}")
        lines.append(f"  TimeDecay: {ev.get('decay_multiplier', 1.0):.3f}")
        lines.append(f"  FINAL:     {ev.get('final', 0):.1f}")

    return f"{action} | 目标仓位 {target * 100:.0f}% | 净评分 {net:+.1f}", "\n".join(lines)


if __name__ == "__main__":
    from data_updater import load_data_from_db

    df = load_data_from_db()
    if df is not None:
        signal, detail = get_today_signal_v6(df)
        print(f"V6.0 今日信号: {signal}")
        print(detail)
