...
5. Behavior Memory 统计（V6.1 增强：含 Psychology 维度）
...
def evaluate_strategy(use_ml=True, emotion_method='weighted', show_plot=None, reference_position=None):
    """V6.2.1 完整策略评估（ML默认启用）

    Args:
        use_ml: 是否启用ML证据源
        emotion_method: 情绪融合方法
        show_plot: 是否显示图表；None 时由命令行 --plot 决定
        reference_position: 信号日建议标记的参考持仓（0-1）；
                            None 时使用回测口径持仓，也可由命令行 --pos 指定
    """
    import sys
    ...
    # V6.2.2: 支持 --pos 指定信号日建议标记的参考持仓（如 python main.py --eval --plot --pos 0.6）
    if reference_position is None and '--pos' in sys.argv:
        try:
            reference_position = float(sys.argv[sys.argv.index('--pos') + 1])
        except (ValueError, IndexError):
            pass

    print("=" * 80)
    print("A500 ETF V6.2.1 Evidence Engine 稳定化版本 回测评估")
    print("=" * 80)
    ...
    # Step 6: V6.1 Behavior Memory + Replay Summary
    ...
    if HAS_MATPLOTLIB and show_plot:
        print("\n[图表生成]")
        plot_results(df, signals, bt, results, reference_position=reference_position)
    elif HAS_MATPLOTLIB:
        print("\n[图表] 已跳过（使用 --plot 启用绘图）")
...
def print_behavior_memory(strategy):
    """V6.1: 打印行为记忆库统计（增强版）"""
    print("\n" + "=" * 80)
    print("V6.1 Behavior Memory (Replay Learning) 统计")
    print("=" * 80)
...
def plot_results(df, signals, bt, results, reference_position=None):
    """绘制 V6.0 信号图

    reference_position: 信号日建议标记的参考持仓（0-1）；None 时用回测口径持仓
    """
    ...
    # V6.2.2: 信号日建议标记（空心样式，与真实成交标记区分）
    # 说明：--signal 的"操作建议"相对参考持仓计算；即使当日未实际成交也在此画出，
    #       默认用回测口径持仓，可用 reference_position/--pos 传入用户实际持仓。
    try:
        from scoring_engine import get_score_to_action
        if bt.daily_signals:
            last_ds = bt.daily_signals[-1]
            sig_date = last_ds.get('date')
            if sig_date is not None and sig_date in df_plot.index:
                ref_pos = reference_position if reference_position is not None \
                    else last_ds.get('current_position', bt.pm.position_pct or 0.5)
                rec_action, rec_target, _ = get_score_to_action(
                    last_ds.get('buy_score', 0), last_ds.get('sell_score', 0), ref_pos)
                rec_price = df_plot.loc[sig_date, 'close']
                rec_label = f"信号建议 {rec_action} {rec_target * 100:.0f}%"
                if rec_action == 'BUY':
                    ax1.scatter(sig_date, rec_price, s=170, marker='^', facecolors='none',
                                edgecolors='#00C853', linewidths=2.2, zorder=6)
                    xytext, color = (0, 30), '#00C853'
                elif rec_action == 'SELL':
                    ax1.scatter(sig_date, rec_price, s=170, marker='v', facecolors='none',
                                edgecolors='#FF1744', linewidths=2.2, zorder=6)
                    xytext, color = (0, -34), '#FF1744'
                else:
                    ax1.scatter(sig_date, rec_price, s=120, marker='o', facecolors='none',
                                edgecolors='#9E9E9E', linewidths=2.2, zorder=6)
                    xytext, color = (0, 24), '#9E9E9E'
                ax1.annotate(rec_label, (sig_date, rec_price), textcoords="offset points",
                             xytext=xytext, ha='center', fontsize=7, fontweight='bold',
...
def print_calibration_evaluation(strategy):
    """V6.1: 打印概率校准评估"""
    print("\n" + "=" * 80)
    print("V6.1 Probability Calibration 评估")
    print("=" * 80)
...
def print_evidence_explainability(bt):
    """V6.1: 打印证据可解释性摘要"""
    print("\n" + "=" * 80)
    print("V6.1 Evidence Explainability 摘要")
    print("=" * 80)
