"""
V6.2.1 策略配置文件 —— Evidence Engine 稳定化版本
=================================================
...
# Behavior 4: False Break（假突破）—— V6.1 阈值放宽
...
# Behavior 6: Trend Failure（趋势衰退）—— V6.1 阈值降低
...
REGIME_WEIGHTS = {
    ...
    'Range': {
        'buy_mult': 0.55,    # V6.2.1: 震荡市中大幅减少买入（交易往往不正确）
        'sell_div': 0.85,    # V6.2.1: 震荡市中卖出适度加强（及时止盈）
    },
    'Unknown': {
        'buy_mult': 0.75,
        'sell_div': 0.95,
...
# 交易执行参数
# ============================================================
MAX_POSITION = 0.98           # 最大仓位98%（heavy优化），牛市中允许更高仓位
MIN_TRADE_VALUE = 50
SLIPPAGE = 0.001
COMMISSION = 0.0003
...
INITIAL_POSITION = 0.89   # 初始仓位89%（heavy优化）

# V6.1: 最低持仓天数
MIN_HOLD_DAYS = 34        # 买入后至少34天才能卖出（heavy优化）

# V6.1: 交易执行阈值
TRADE_TARGET_DELTA = 0.091  # 目标仓位变化超过9.1%才执行（heavy优化）
TRADE_ACTUAL_DELTA = 0.019  # 实际仓位偏离目标超过1.9%才执行（heavy优化）
SCORE_HOLD_ZONE = 17       # HOLD区间（heavy优化）
...
EVIDENCE_ENABLE_ML = True         # V6.1: ML默认启用
EVIDENCE_ENABLE_EMOTION_BONUS = True  # 情绪双确认

# ============================================================
# V6.1 升级：Replay Learning (Behavior Memory) 参数
# ============================================================
REPLAY_WINDOW_DAYS = 90           # 滑动窗口大小（交易日）
REPLAY_MIN_SAMPLES = 3            # 最小样本量（V6.1: 降低到3，配合Laplace平滑）
...
# 成功率 → 置信度乘数映射
REPLAY_HIGH_SUCCESS = 0.65        # 成功率高于此值 → 加分（V6.1调低到65%）
REPLAY_LOW_SUCCESS = 0.35         # 成功率低于此值 → 减分（V6.1调低到35%）
REPLAY_CONSECUTIVE_FAIL = 4       # 连续N次失败 → 紧急降权（V6.1放宽到4）

# ============================================================
# V6.1 升级：Time Decay 参数（关键修复）
# ============================================================
# V6.0问题: 衰减过于激进，所有置信度几乎全部压缩到0.1
# V6.1修复: 使用exp(-days/τ)平滑衰减，输出范围[0.5, 1.0]
TIME_DECAY_GRACE_PERIOD = 5       # 前N天不衰减（宽限期）
TIME_DECAY_TAU = 90               # V6.1: 衰减时间常数τ（天），越大衰减越慢
TIME_DECAY_MIN_MULTIPLIER = 0.7   # V6.1: 最小衰减乘数（提升到0.7，保留更多置信度）
TIME_DECAY_MIN_CONFIDENCE = 15    # 衰减到此值以下 → 过期标记（V6.1调低到15）

# ============================================================
# V6.0 新增：EmotionBuilder 参数
# ============================================================
...
# ============================================================
# V6.1 升级：ML Confidence 参数
# ============================================================
ML_MODEL_TYPE = 'rf'              # 模型类型: 'rf' | 'gb' | 'lr'
ML_N_ESTIMATORS = 100             # 树的数量
ML_MAX_DEPTH = 5                  # 最大深度（防过拟合）
ML_MIN_TRAINING_SAMPLES = 30      # V6.1: 降低到30（更早开始学习）
ML_CV_FOLDS = 5                   # 交叉验证折数
ML_RETRAIN_FREQUENCY = 10         # V6.1: 每N笔交易后重新训练ML

# ============================================================
# V6.1 升级：Probability Calibration 参数
# ============================================================
CALIBRATION_METHOD = 'platt'      # 校准方法: 'platt' | 'isotonic' | 'temperature'
CALIBRATION_MIN_SAMPLES = 30      # V6.1: 降低校准样本门槛
CALIBRATION_AUTO_DISABLE = True   # V6.1: 若校准变差则自动禁用

# ============================================================
# V6.1 新增：Evidence Explainability 参数
# ============================================================
