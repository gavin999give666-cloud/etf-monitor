...
class GridSearch:
    ...
    def __init__(self, df, start_date=None, checkpoint_path=None):
        self.df = df
    ...
    def apply_best_params(self, metric='calmar_ratio', dry_run=False):
        """
        将网格搜索的最优参数写回 config.py

        Args:
            metric: 优化目标指标（默认 calmar_ratio）
            dry_run: True=仅打印不写入，False=实际写入

        Returns:
            写入的参数字典，若无可写入结果则返回 None
        """
        import re

        best = self.get_best_params(metric)
        if not best:
            print("[apply_best_params] 无搜索结果，跳过写入")
            return None

        config_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'config.py')
        if not os.path.exists(config_path):
            print(f"[apply_best_params] config.py 未找到: {config_path}")
            return None
        ...
        updated = {}
        for param_name, new_value in best.items():
            old_val = getattr(config, param_name, None)
            if old_val is None:
                continue

            # 匹配形如 "PARAM_NAME = 旧值" 的行，保留后面的注释
            pattern = rf'^({param_name}\s*=\s*)([^\n#]+)(.*)$'
            match = re.search(pattern, source, re.MULTILINE)
            if not match:
                print(f"  [跳过] {param_name}: 在 config.py 中未找到匹配行")
                continue

            # 保持原值格式（float/int/负数的格式保持一致）
            ...
            replacement = f'{param_name} = {new_str}{match.group(3)}'
            source = source[:match.start()] + replacement + source[match.end():]
            updated[param_name] = {'old': old_val, 'new': new_value}

        if not updated:
            print("[apply_best_params] 没有参数需要更新")
            return None

        # 打印变更摘要
        print(f"\n{'='*60}")
        print(f"  应用最优参数到 config.py（优化目标: {metric}）")
        print(f"{'='*60}")
        for name, vals in updated.items():
            old_fmt = f"{vals['old']}"
            new_fmt = f"{vals['new']}"
            arrow = "→"
            print(f"  {name:<32} {old_fmt:<10} {arrow} {new_fmt}")

        if dry_run:
            print(f"\n  [DRY RUN] 未实际写入 config.py")
            return updated
...
def _auto_shutdown_prompt(seconds=10):
    """
    网格搜索完成后弹窗提示自动关机

    弹窗显示倒计时，提供"取消关机"按钮。
    若用户在倒计时内未取消，则自动执行关机（无人值守模式）。
    """
    ...
    def do_shutdown():
        if not shutdown_cancelled.is_set():
            print(f"\n[自动关机] 用户未响应，{seconds}秒倒计时结束，正在关机...")
            os.system('shutdown /s /t 0')

    timer = threading.Timer(seconds, do_shutdown)
    timer.start()
    ...
    def update_label():
        if not shutdown_cancelled.is_set():
            label_var.set(f"网格搜索已完成！\n\n系统将在 {remaining[0]} 秒后自动关机")
            if remaining[0] > 0:
                remaining[0] -= 1
    ...
    # 标题
    title_label = tk.Label(root, text="网格搜索完成", font=("Microsoft YaHei", 14, "bold"))
    title_label.pack(pady=(15, 5))

    # 倒计时标签
    label_var = tk.StringVar()
    label_var.set(f"网格搜索已完成！\n\n系统将在 {seconds} 秒后自动关机")
    countdown_label = tk.Label(root, textvariable=label_var, font=("Microsoft YaHei", 11),
    ...
    cancel_btn = tk.Button(btn_frame, text="取消关机", command=cancel_shutdown,
                           font=("Microsoft YaHei", 11), bg="#4CAF50", fg="white",
                           width=12, height=1, cursor="hand2")
    cancel_btn.pack(side=tk.LEFT, padx=5)

    shutdown_now_btn = tk.Button(btn_frame, text="立即关机", command=lambda: (timer.cancel(), do_shutdown()),
                                 font=("Microsoft YaHei", 11), bg="#F44336", fg="white",
                                 width=12, height=1, cursor="hand2")
    shutdown_now_btn.pack(side=tk.LEFT, padx=5)

    # 启动倒计时
    root.after(1000, update_label)

    # 窗口关闭按钮 = 取消关机
    root.protocol("WM_DELETE_WINDOW", cancel_shutdown)
...
def run_grid_search_full(n_jobs=-1, resume=True):
    ...
    # 自动将最优参数写回 config.py（默认按 Calmar Ratio 优化）
    searcher.apply_best_params(metric='calmar_ratio')

    # 网格搜索完成后弹窗提示自动关机（无人值守）
    _auto_shutdown_prompt(seconds=10)
...
if __name__ == "__main__":
    if '--full' in sys.argv:
        ...
        # 手动将已有搜索结果写入 config.py（不重新搜索）
        metric = 'calmar_ratio'
        for m in ['strategy_return', 'sharpe_ratio', 'calmar_ratio']:
            if f'--{m}' in sys.argv:
                metric = m
                break
        ...
        if df is not None:
            searcher = GridSearch(df)
            ckpt = searcher._load_checkpoint_meta()
            if ckpt and ckpt.get('results'):
                searcher.results = ckpt['results']
                print(f"从断点加载了 {len(searcher.results)} 条结果")
                searcher.apply_best_params(metric=metric, dry_run=dry_run)
            else:
                print("未找到断点文件，请先运行 --full 完成网格搜索")
