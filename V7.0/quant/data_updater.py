"""
V6.2.3 数据更新模块（V7.0 多标的版）
Sina API(主) -> baostock(备) -> akshare(末备)
最稳定免费的 A 股/ETF 日线数据获取方案

V7.0 变更：
- 数据库按标的隔离：data\{code}.db（config.DATA_FILE），由 activate_profile 派生。
- 数据源前缀（sh./sz.）从 config.MARKET 取值，支持创业板等 sz 标的。
"""
import pandas as pd
import requests
import json
...
def get_db_path():
    """获取当前标的的数据库文件路径（V7.0：data\{code}.db，按标的隔离）"""
    ...
    if data_file and os.path.exists(data_file):
        return data_file
    # 兼容：路径尚未存在时直接返回目标路径（更新流程首次建库）
    if data_file:
        return data_file
    # 兜底：回退到模块目录 stock_data.db（冷启动/异常场景）
...
def migrate_db(db_path=None):
    """对已有数据库执行迁移（添加新列）；表不存在时自动兜底建表"""
    if db_path is None:
        db_path = get_db_path()
    conn = sqlite3.connect(db_path)
    try:
        has_table = conn.execute(
            "SELECT 1 FROM sqlite_master WHERE type='table' AND name='stock_data'"
        ).fetchone()
        if not has_table:
            init_database(db_path)
            print("数据库初始化完成：stock_data 表不存在，已按新 schema 自动创建")
            return
...
# 数据源 1: Sina 财经 API（主）
# ============================
def fetch_from_sina(stock_code=None, start_date=None, end_date=None, scale=240, datalen=2000):
    """从新浪财经获取 K 线数据（scale=240为日线，scale=5为5分钟线）"""
    if stock_code is None:
        stock_code = config.STOCK_CODE
    market = str(getattr(config, 'MARKET', 'sh'))
...
# 数据源 2: baostock（备）
# ============================
def fetch_from_baostock(stock_code=None, start_date=None, end_date=None):
    """从 baostock 获取日线数据"""
    ...
    # 优先用 config.MARKET（支持 159915 等 sz 标的）；缺失时按代码首字推断
    market = str(getattr(config, 'MARKET', ''))
    if market in ('sh', 'sz'):
        bs_code = f'{market}.{code}'
    elif code.startswith('6') or code.startswith('58'):
        bs_code = f'sh.{code}'
...
# 数据源 3: akshare（末备）
# ============================
def fetch_from_akshare(stock_code=None, start_date=None, end_date=None):
...
# 统一获取入口
# ============================
def fetch_stock_data(stock_code=None, start_date=None, end_date=None):
    """多数据源自动切换获取日线数据"""
    if stock_code is None:
        stock_code = config.STOCK_CODE
...
def update_stock_data(stock_code=None):
    """增量更新数据"""
    if stock_code is None:
        stock_code = config.STOCK_CODE
    db_path = get_db_path()
...
def full_refresh_data(stock_code=None):
    """完全刷新数据"""
    if stock_code is None:
        stock_code = config.STOCK_CODE
    db_path = get_db_path()
