...
def evaluate_strategy(use_ml=True, emotion_method='weighted', show_plot=None):
    ...
    print("=" * 80)
    print(f"{config.ETF_NAME} Evidence Engine 稳定化版本 回测评估")
    print("=" * 80)
...
def plot_results(df, signals, bt, results):
    ...
    fig.suptitle(f'{config.ETF_NAME} ({config.STOCK_CODE}) Evidence Engine Strategy', fontsize=16, fontweight='bold')
