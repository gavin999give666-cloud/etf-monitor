"""
V6.2.3 Replay Learning —— 行为成功率统计（在线学习闭环）
========================================================
...
class BehaviorMemory:
    ...
    def __init__(self, window_days=90, min_samples=3, max_age_days=30,
        ...
        Args:
            ...
            min_samples: 最小样本量（V6.2.3降低到3，配合Laplace平滑）
        ...
        # V6.2.3: 核心数据结构升级 —— (regime, behavior, psychology) → list of (date, success)
        ...
        # V6.2.3: 全局先验成功率（用于Laplace平滑）
        self._global_successes = 0
        self._global_total = 0

    # ============================================================
    # V6.2.3: 构建多维 Replay 键
    # ============================================================
    ...
    def record_trade(self, regime, behavior_name, success, date, psychology=None):
        """
        V6.2.3: 记录一笔交易的结果（增加 Psychology 维度）
    ...
    def get_success_rate(self, regime, behavior_name, psychology=None, current_date=None):
        """
        V6.2.3: 获取 (Regime, Behavior, Psychology) 组合的时间加权 + Laplace平滑成功率

        Args:
            regime: 市场状态
            behavior_name: 行为名称
            psychology: 情绪状态（V6.2.3新增）
    ...
    def get_confidence_multiplier(self, regime, behavior_name, current_date=None,
                                   psychology=None):
        """
        V6.2.3: 获取基于历史成功率的置信度修正系数

        改进:
        - 多维键支持 Psychology
        - 时间加权成功率
        - Laplace平滑避免小样本误导
        - 连续4次失败 → 紧急降权（V6.2.3放宽到4）

        逻辑：
        - 成功率 > 65% → 加分（V6.2.3调低阈值）
        - 成功率 < 35% → 减分（V6.2.3调低阈值）
        - 样本不足 → 系数 1.0（中性）
        - 连续4次失败 → 系数 0.70
        - 超过30天无新样本 → 回归1.0
    ...
    def get_replay_summary(self):
        """
        V6.2.3: 获取 Replay 学习摘要（Top10 + Worst10）
    ...
    def print_stats(self):
        ...
        sep = "=" * 95
        print(sep)
        print("V6.2.3 Behavior Memory (Replay Learning) Statistics")
    ...
    def print_replay_summary(self):
        """V6.2.3: 打印 Replay Learning Top10 / Worst10"""
        ...
        print("\n" + "=" * 70)
        print("V6.2.3 Replay Learning Summary")
...
# ============================================================
# V6.2.3: Time Decay 模块（关键修复）
# ============================================================

class TimeDecay:
    """
    V6.2.3 观察期置信度时间衰减（修复版）
    ...
    def print_decay_curve(self, max_days=30):
        """打印衰减曲线"""
        print("\nV6.2.3 Time Decay Curve (exp(-days/τ), τ={}):".format(self.tau))
...
if __name__ == "__main__":
    # 演示 V6.2.3 BehaviorMemory
    import random
    random.seed(42)

    print("=" * 60)
    print("V6.2.3 BehaviorMemory Demo")
    print("=" * 60)
    ...
    # V6.2.3 Time Decay 演示
    print("\n" + "=" * 60)
    print("V6.2.3 Time Decay Demo (exp(-days/τ), τ=90)")
    print("=" * 60)
