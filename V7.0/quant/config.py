"""
V6.2.3 策略配置文件 —— Evidence Engine 稳定化版本（V7.0 多标的化）
=================================================================

V7.0 变更：本文件为"默认参数" + Profile 激活机制。
- 全部策略决策参数默认值在此定义（= A500 基线，即 _default.json 冷启动参数）。
- 特定标的通过 activate_profile(code) 加载 profiles\{code}.json 后 setattr 覆盖。
- 派生值（STOCK_CODE / DB 路径 / runs 路径 / 名称）随激活标的自动更新。
...
# ============================================================
# 基础配置（默认标的；activate_profile 后由 profile 覆盖）
# ============================================================
STOCK_CODE = '563360'
DB_FILENAME = 'stock_data.db'
ETF_NAME = '中证 A500 ETF'          # 默认标的名称（profile name）
MARKET = 'sh'                        # 数据源交易所前缀 sh/sz
OPTIMIZED = True                     # 默认标的视为已优化

# V7.0：路径管理（多标的隔离）
CONFIG_DIR = os.path.dirname(os.path.abspath(__file__))      # ...\V7.0\quant
V7_ROOT = os.path.dirname(CONFIG_DIR)                        # ...\V7.0
PROFILES_DIR = os.path.join(V7_ROOT, 'profiles')
DATA_DIR = os.path.join(V7_ROOT, 'data')
RUNS_ROOT = os.path.join(V7_ROOT, 'runs')
DATA_FILE = os.path.join(DATA_DIR, f'{STOCK_CODE}.db')       # data\{code}.db
RUNS_DIR = os.path.join(RUNS_ROOT, STOCK_CODE)               # runs\{code}\
CURRENT_PROFILE = None
CURRENT_PROFILE_CODE = None
...
# 空心建议净分阈值：净评分(买-卖) 须突破 ±该值才画空心建议标记（仅影响回测图标记，不影响实盘交易判定）
REC_MARKER_NET_ZONE = 45
...
# ============================================================
# V7.0 多标的：Profile 激活机制
# ============================================================

def available_profiles():
    """列出 profiles 目录下所有 profile 代码（去 DEFAULT / _default）"""
...
def load_profile(code):
    ...
    if not os.path.exists(path):
        raise FileNotFoundError(f"Profile 不存在: {path} （可用: {available_profiles()}）")
    with open(path, encoding='utf-8') as f:
        return json.load(f)


def _sync_derived(profile):
    """激活后同步派生值（代码 / 名称 / DB / runs 路径）"""
    global CURRENT_PROFILE, CURRENT_PROFILE_CODE, CURRENT_NAME_CN
    code = profile.get('code', '')
    if not code or code == 'DEFAULT':
        code = STOCK_CODE
    CURRENT_PROFILE_CODE = code
    CURRENT_PROFILE = profile
    globals()['STOCK_CODE'] = code
    if profile.get('name'):
        globals()['ETF_NAME'] = profile['name']
    if profile.get('market'):
        globals()['MARKET'] = profile['market']
    if 'optimized' in profile:
        globals()['OPTIMIZED'] = bool(profile['optimized'])
    globals()['DATA_FILE'] = os.path.join(DATA_DIR, f'{code}.db')
    globals()['RUNS_DIR'] = os.path.join(RUNS_ROOT, code)
    globals()['DB_FILENAME'] = f'{code}.db'
    os.makedirs(DATA_DIR, exist_ok=True)
    os.makedirs(RUNS_DIR, exist_ok=True)


def activate_profile(code):
    """激活指定标的的策略参数：读取 profiles\{code}.json → setattr 到 config 模块全局。

    两条铁律（见设计方案 §3.1）：
    1. 主进程同一时刻只激活一个标的；切换标的 = 重新 activate + 重建策略实例。
    2. 所有重计算（优化/回测）在子进程跑，子进程入口统一 --profile {code} 先激活自己。
    """
    profile = load_profile(code)
    for key, value in profile.items():
        # 数字类型：profile JSON 中整数以 int 存储，保持原样（float 均为 float）
        setattr(config_module_self(), key, value)
    _sync_derived(profile)
    return profile


def get_current_profile():
    """返回当前激活的 profile dict（未激活返回 None）"""
    return CURRENT_PROFILE


def config_module_self():
