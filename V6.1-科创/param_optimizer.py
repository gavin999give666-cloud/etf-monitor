"""
V6.1 智能参数优化器（Smart Parameter Optimizer）
==================================================
...
class HeavyOptimizer:
    ...
    def run(self, n_trials=2000, ga_generations=100, ga_population=60,
        ...
        if 'optuna' in self._completed_phases:
            ...
            if remaining > 0:
                # 分批运行 + 定期保存 pickle checkpoint（防断电丢失 journal）
                # JournalFileBackend 断电时可能丢失未刷盘的数据，所以每 batch_size 个 trials 保存一次 pickle
                _batch_size = 50
                _last_save_count = 0
                while remaining > 0:
                    _batch = min(_batch_size, remaining)
                    study.optimize(evaluator, n_trials=_batch, n_jobs=n_jobs,
                                   show_progress_bar=HAS_TQDM and verbose)
                    remaining -= _batch

                    # 每 100 个 trials 保存一次 pickle checkpoint
                    _cur_complete = sum(1 for t in study.trials
                                        if t.state == optuna.trial.TrialState.COMPLETE)
                    if _cur_complete - _last_save_count >= 100:
                        self._save_phase1_results_to_pickle(study)
                        _last_save_count = _cur_complete
                        print(f"\n  [checkpoint] 已保存 {_cur_complete}/{n_trials} trials 到 pickle")
            ...
            if verbose and phase1_results:
                ...
                try:
                    ...
                    print(f"  Top-5 重要参数: {', '.join(list(imp.keys())[:5])}")
                except:
                    pass
        ...
        self._export_results_json(total_elapsed)

    def _export_results_json(self, total_elapsed):
        """导出所有结果到 heavy_results.json"""
        import json as _json
...
def run_heavy_optimization(resume=True, **kwargs):
    """全量高算力优化入口（支持断点续算）"""
    df = load_data_from_db()
    if df is None:
        print("无法加载数据")
    ...
    heavy.run(resume=resume, **kwargs)
    return heavy
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='V6.1 智能参数优化器')
