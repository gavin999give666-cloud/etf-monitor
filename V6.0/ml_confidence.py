...
def build_training_data_from_trades(trades, feature_builder_fn):
    """
    从交易记录构建ML训练数据
    ...
    Args:
        trades: list of dict，每笔交易包含:
            ...
            - 'entry_risk': float
            - 'pnl_pct': float
        feature_builder_fn: function(trade) → feature_vector
...
def default_feature_builder(trade):
    ...
    从交易记录提取特征向量，与Evidence Engine的_build_ml_features一致。
    """
    behavior_types = [
    ...
    behavior_onehot = [1 if main_behavior == b else 0 for b in behavior_types]
    regime_onehot = [1 if trade.get('entry_regime') == r else 0 for r in regime_types]
    emotion_onehot = [1 if trade.get('entry_psychology') == e else 0 for e in emotion_types]
