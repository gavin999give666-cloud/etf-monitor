...
from strategy import V5Strategy
from backtest import V5Backtest
...
def evaluate_strategy():
    """V5.0 完整策略评估"""
    print("=" * 80)
    print("A500 ETF V5.0 行为生命周期 + 市场心理策略 回测评估")
    print("=" * 80)
    ...
    # Step 2: 运行回测
    print("[2/6] 运行增强回测引擎...")
    bt = V5Backtest(df, start_date='2025-09-01')
...
def print_psychology_distribution(bt):
    """打印情绪状态分布"""
    print("\n" + "=" * 80)
    print("市场情绪状态分布 (Crowd Psychology)")
    print("=" * 80)
...
def plot_results(df, signals, bt, results):
    ...
    for s in signals:
        if s['date'] < pd.Timestamp('2025-09-01'):
            continue
        if s['confirmed_buy_events'] > 0 and s['date'] in df_plot.index:
            buy_dates.append(s['date'])
            buy_prices.append(df_plot.loc[s['date'], 'close'])
            buy_labels.append(s['buy_behaviors'][0] if s['buy_behaviors'] else 'Buy')
        if s['confirmed_sell_events'] > 0 and s['date'] in df_plot.index:
            sell_dates.append(s['date'])
            sell_prices.append(df_plot.loc[s['date'], 'close'])
            sell_labels.append(s['sell_behaviors'][0] if s['sell_behaviors'] else 'Sell')

    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(18, 10),
                                     gridspec_kw={'height_ratios': [3, 1]}, sharex=True)
    fig.suptitle('A500 ETF (563360) V5.0 Behavior Lifecycle Strategy', fontsize=16, fontweight='bold')
