"""
V6.2.3 策略流水线（Strategy Pipeline）—— 稳定化版本
===================================================
...
完整的 V6.2.3 架构：
  ...
  Event Engine + Lifecycle（event_engine.py）
        ↓
  Evidence Engine（evidence_engine.py）—— V6.2.3 在线学习闭环
        ↓
  ML Confidence（ml_confidence.py）—— V6.2.3 真正启用
        ↓
  Probability Calibration（probability_calibration.py）—— V6.2.3 评估
        ↓
...
class V6Strategy:
    ...
    def run(self, df, start_date=None):
        ...
        for i in range(len(df)):
            ...
            # V6.2.3: 在牛市中买入评分突然大幅下降 → 视为止盈信号，开启直入窗口
    ...
    def _compute_v6_score(self, reward_score, risk_score, psych_state, regime,
        ...
        """
        V6.2.3 评分计算 + V6.2.3 历史高位保护 + V6.2.3 牛市直入
    ...
    def record_trade_result(self, regime, behavior_name, success, date, psychology=None):
        """
        V6.2.3: 记录交易结果到行为记忆库（增加 Psychology 维度）
    ...
    def _train_ml_model(self):
        """V6.2.3: 训练ML模型和校准器（使用统一 FeatureBuilder）"""
...
def get_today_signal_v6(df):
    ...
    """
    if len(df) < 60:
        return "0%", "数据不足，需要至少60天数据（V6.2.3）"
    ...
    lines.append("=" * 60)
    lines.append("科创综指 ETF V6.2.3 Evidence Engine 策略")
    lines.append("=" * 60)
