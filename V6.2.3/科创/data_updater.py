"""
V6.2.3 数据更新模块（合并版：data_updater + data_updater_baostock）
...
def get_db_path():
    ...
    for ver in ['V6.2.3', 'V6.2.3', 'V5.0', 'V4.0', 'V2.0', 'V1.0']:
...
def load_data_from_db(db_path=None):
    ...
    if db_path is None:
        db_path = get_db_path()
    migrate_db(db_path)  # 确保表存在（不存在时自动建表）
    conn = sqlite3.connect(db_path)
