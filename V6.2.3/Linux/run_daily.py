...
logger = logging.getLogger('V6.2.3_Daily')
...
def step_bark_push(signal_text, detail_text, signal_date_str, cfg):
    ...
    title = f"A500 ETF V6.2.3 信号 {signal_date_str}"
    ...
    try:
        ...
        req.add_header('User-Agent', 'V6.2.3-DailyRunner/1.0')
...
def run_daily(test_mode=False, no_bark=False):
    ...
    logger.info("=" * 60)
    logger.info(f"V6.2.3 每日任务启动 | 时间: {start_time.strftime('%Y-%m-%d %H:%M:%S')}")
